...
/**
 * Run the normalize + validate pipeline on already-extracted resume text.
 * Pure/synchronous: callers that resolve text from a file (see
 * src/lib/ai/resume-text.ts) reuse this instead of re-deriving from structured
 * sections only.
 */
export const preprocessResumeText = (rawText: string): PreprocessingResult => {
  try {
